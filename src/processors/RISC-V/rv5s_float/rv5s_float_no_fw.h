...
class RV5S_FLOAT_NO_FW : public RipesVSRTLProcessor {
...
public:
  ...
  RV5S_FLOAT_NO_FW(const ExtensionSetInfo &extensions)
      : RipesVSRTLProcessor(
            "5-Stage floating point RISC-V Processor with hazard detection "
            "but without forwarding") {
    ...
    // -----------------------------------------------------------------------
    // Register source MUXes (no forwarding - values come straight from RF)
    ...
    // -----------------------------------------------------------------------
    // Branch (no forwarding - values come from IDEX register)
    ...
    // -----------------------------------------------------------------------
    // ALU (no forwarding - values come from IDEX register)
    ...
    // -----------------------------------------------------------------------
    // FPU (no forwarding - values come from IDEX register)
    idex_reg->f1_out >> fpu->op1;
    idex_reg->f2_out >> fpu->op2;
