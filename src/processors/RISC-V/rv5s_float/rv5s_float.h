...
#include "rv5s_float_exmem.h"
#include "rv5s_float_forwardingunit.h"
#include "rv5s_float_hazardunit.h"
#include "rv5s_float_idex.h"
#include "rv5s_float_memwb.h"
...
class RV5S_FLOAT : public RipesVSRTLProcessor {
...
public:
  ...
  RV5S_FLOAT(const ExtensionSetInfo &extensions)
      : RipesVSRTLProcessor(
            "5-Stage floating point RISC-V Processor with forwarding and "
            "hazard detection") {
    RV_ExtensionSet exts(extensions);
    exts << Extension::F; // enforce F extension
    ...
    // -----------------------------------------------------------------------
    // Forwarding multiplexers (integer path)
    registerFileI->r1_out >> r1_fw_src->get(ForwardingSrc::IdStage);
    exmem_reg->alures_out >> r1_fw_src->get(ForwardingSrc::MemStage);
    reg_wr_src->out >> r1_fw_src->get(ForwardingSrc::WbStage);
    funit->alu_reg1_forwarding_ctrl >> r1_fw_src->select;

    registerFileI->r2_out >> r2_fw_src->get(ForwardingSrc::IdStage);
    exmem_reg->alures_out >> r2_fw_src->get(ForwardingSrc::MemStage);
    reg_wr_src->out >> r2_fw_src->get(ForwardingSrc::WbStage);
    funit->alu_reg2_forwarding_ctrl >> r2_fw_src->select;

    // -----------------------------------------------------------------------
    // Forwarding multiplexers (float path)
    registerFileF->r1_out >> f1_fw_src->get(ForwardingSrc::IdStage);
    exmem_reg->alures_out >> f1_fw_src->get(ForwardingSrc::MemStage);
    reg_wr_src->out >> f1_fw_src->get(ForwardingSrc::WbStage);
    funit->fpu_reg1_forwarding_ctrl >> f1_fw_src->select;

    registerFileF->r2_out >> f2_fw_src->get(ForwardingSrc::IdStage);
    exmem_reg->alures_out >> f2_fw_src->get(ForwardingSrc::MemStage);
    reg_wr_src->out >> f2_fw_src->get(ForwardingSrc::WbStage);
    funit->fpu_reg2_forwarding_ctrl >> f2_fw_src->select;

    registerFileF->r3_out >> f3_fw_src->get(ForwardingSrc::IdStage);
    exmem_reg->alures_out >> f3_fw_src->get(ForwardingSrc::MemStage);
    reg_wr_src->out >> f3_fw_src->get(ForwardingSrc::WbStage);
    funit->fpu_reg3_forwarding_ctrl >> f3_fw_src->select;

    // -----------------------------------------------------------------------
    // Register source MUXes (after forwarding)
    r1_fw_src->out >> reg1_src->get(ImmRegFileSrc::INTEGER);
    f1_fw_src->out >> reg1_src->get(ImmRegFileSrc::FLOAT);
    immediate->imm >> reg1_src->get(ImmRegFileSrc::IMMEDIATE);
    control->reg1_do_read_src >> reg1_src->select;

    r2_fw_src->out >> reg2_src->get(RegFileSrc::INTEGER);
    f2_fw_src->out >> reg2_src->get(RegFileSrc::FLOAT);
    control->reg2_do_read_src >> reg2_src->select;

    // -----------------------------------------------------------------------
    // Branch (uses forwarded integer register values)
    idex_reg->br_op_out >> branch->comp_op;
    r1_fw_src->out >> branch->op1;
    r2_fw_src->out >> branch->op2;
    ...
    // -----------------------------------------------------------------------
    // ALU (uses forwarded integer register values)
    r1_fw_src->out >> alu_op1_src->get(AluSrc1::REG1);
    idex_reg->pc_out >> alu_op1_src->get(AluSrc1::PC);
    idex_reg->alu_op1_ctrl_out >> alu_op1_src->select;

    r2_fw_src->out >> alu_op2_src->get(AluSrc2::REG2);
    ...
    // -----------------------------------------------------------------------
    // FPU (uses forwarded float register values)
    f1_fw_src->out >> fpu->op1;
    f2_fw_src->out >> fpu->op2;
    f3_fw_src->out >> fpu->op3;
    ...
    control->fpu_ctrl >> idex_reg->fpu_ctrl_in;
    ...
    alu_fpu_src->out >> exmem_reg->alures_in;
    ...
    idex_reg->regF_do_write_out >> exmem_reg->regF_do_write_in;
    ...
    exmem_reg->regF_do_write_out >> memwb_reg->regF_do_write_in;
    ...
    decode->r3_reg_idx >> funit->id_reg3_idx;

    exmem_reg->wr_reg_idx_out >> funit->mem_int_wr_idx;
    exmem_reg->reg_do_write_out >> funit->mem_int_wr_en;
    exmem_reg->wr_reg_idx_out >> funit->mem_flt_wr_idx;
    exmem_reg->regF_do_write_out >> funit->mem_flt_wr_en;

    memwb_reg->wr_reg_idx_out >> funit->wb_int_wr_idx;
    memwb_reg->reg_do_write_out >> funit->wb_int_wr_en;
    memwb_reg->wr_reg_idx_out >> funit->wb_flt_wr_idx;
    memwb_reg->regF_do_write_out >> funit->wb_flt_wr_en;

    // -----------------------------------------------------------------------
    // Hazard detection unit
    decode->r1_reg_idx >> hzunit->id_reg1_idx;
    decode->r2_reg_idx >> hzunit->id_reg2_idx;
    control->reg1_do_read_src >> hzunit->id_reg1_src;
    control->reg2_do_read_src >> hzunit->id_reg2_src;
    ...
    idex_reg->regF_do_write_out >> hzunit->ex_regF_do_write;
    ...
    exmem_reg->regF_do_write_out >> hzunit->mem_regF_do_write;

    memwb_reg->reg_do_write_out >> hzunit->wb_do_reg_write;
    memwb_reg->regF_do_write_out >> hzunit->wb_regF_do_write;
  }
  ...
  SUBCOMPONENT(idex_reg, TYPE(RV5S_FLOAT_IDEX<XLEN>));
  SUBCOMPONENT(exmem_reg, TYPE(RV5S_FLOAT_EXMEM<XLEN>));
  SUBCOMPONENT(memwb_reg, TYPE(RV5S_FLOAT_MEMWB<XLEN>));

  // Multiplexers - Register source selection
  ...
  // Multiplexers - ALU/FPU operand selection
  SUBCOMPONENT(alu_op1_src, TYPE(EnumMultiplexer<AluSrc1, XLEN>));
  SUBCOMPONENT(alu_op2_src, TYPE(EnumMultiplexer<AluSrc2, XLEN>));

  // Multiplexers - Forwarding
  SUBCOMPONENT(r1_fw_src, TYPE(EnumMultiplexer<ForwardingSrc, XLEN>));
  SUBCOMPONENT(r2_fw_src, TYPE(EnumMultiplexer<ForwardingSrc, XLEN>));
  SUBCOMPONENT(f1_fw_src, TYPE(EnumMultiplexer<ForwardingSrc, XLEN>));
  SUBCOMPONENT(f2_fw_src, TYPE(EnumMultiplexer<ForwardingSrc, XLEN>));
  SUBCOMPONENT(f3_fw_src, TYPE(EnumMultiplexer<ForwardingSrc, XLEN>));

  // Multiplexers - Write back and PC
  ...
  SUBCOMPONENT(funit, ForwardingUnit_FLOAT);
  SUBCOMPONENT(hzunit, HazardUnit_FLOAT);
